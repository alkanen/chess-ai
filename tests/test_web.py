...
def build_frontend(static_dir: Path) -> Path:
    """Write a stand-in for the built frontend."""
    (static_dir / "assets").mkdir(parents=True)
    (static_dir / "index.html").write_text(INDEX_HTML, encoding="utf-8")
    (static_dir / "assets" / "index.js").write_text("console.log('board');", encoding="utf-8")
...
def static_dir(tmp_path):
    return build_frontend(tmp_path / "static")
...
def test_unbuilt_frontend_is_explained_but_api_works(tmp_path):
    ...
    assert unbuilt_client.get("/chess/assets/index.js").status_code == 404
    assert unbuilt_client.get("/chess/api/start-position").status_code == 200


def test_frontend_built_after_start_is_served_without_restart(tmp_path):
    static_dir = tmp_path / "static"
    chess_client = client("/chess", static_dir)
    assert chess_client.get("/chess/").status_code == 503

    build_frontend(static_dir)

    assert chess_client.get("/chess/").status_code == 200
    assert chess_client.get("/chess/assets/index.js").text == "console.log('board');"
